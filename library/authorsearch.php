<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $query = mysqli_query($link, "SELECT * FROM itlabexerciselibrary WHERE AUTHOR = '$author'");
        ///// If it does the program will continue
        if(mysqli_num_rows($query) == 1){
        /// this is to fetch the row that contains the ID that is to be used
        echo '  <script> alert("A Book by '. $author.' exists");  window.location.href = "./searchbook-page.php"</script>';
    
        }
        else{
            /// if the current Isbn doesn't exist it shows an alert error 
            echo '  <script> alert("A Book by '. $author.' does NOT exist");  window.location.href = "./searchbook-page.php"</script>';
        }
    }
