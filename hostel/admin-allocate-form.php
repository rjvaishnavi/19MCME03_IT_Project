        ...
        $query = mysqli_query($link, "SELECT * FROM itlabexerciserooms WHERE ROOM = '$roomno'"); 
            ...
            /// this is to fetch the row that contains the ID that is to be used
                $row = mysqli_fetch_assoc($query);
                    ...
                    // it creates a delete 
                    $edit = mysqli_query($link,"UPDATE itlabexerciserooms SET VACANCY='$stat', HOSTELLER ='$id' WHERE ROOM ='$roomno' ");
                    // if the query is sucessful, it will show this alert and move to the admin page
                    if($edit){
                        echo '<script> alert("The Room has been allocated!");  window.location.href = "./hostel_admin_page.php"</script>';

                    }
                    else{
                        // else it will show an error alert
                    echo '  <script> alert("Failed to Allocate room");  window.location.href = "./admin-allocate.php"</script>';
                    }
        
            }
            else{
                /// if the current Isbn doesn't exist it shows an alert error 
            echo '  <script> alert("The Room number does not exist");  window.location.href = "./admin-allocate.php"</script>';
            }    
        }
        else{
            /// if the current ID doesn't exist it shows an alert error 
        echo '  <script> alert("The ID does not exist");  window.location.href = "./admin-allocate.php"</script>';
        }
    }
